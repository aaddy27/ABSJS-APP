...
class _RamDarshanScreenState extends State<RamDarshanScreen> {
  static const String _cacheKey = 'ram_darshan_books_cache_v1';
  static const String _imagesDirName = 'ram_darshan_covers';
  ...
  Future<void> _loadCacheThenRefresh() async {
    await _loadCache();
    // start background refresh but don't block UI
    _refreshFromNetwork();
  }
  ...
  Future<void> _refreshFromNetwork({bool showSnackOnError = false}) async {
    ...
    try {
      final res = await http.get(Uri.parse('https://website.sadhumargi.in/api/sahitya/category/ram_darshan'));
      if (res.statusCode == 200) {
        final List<dynamic> fetched = jsonDecode(res.body);

        // Compare by IDs to detect changes
        ...
        // Ensure cover images are cached locally (downloads missing ones)
        _ensureImagesForBooks(fetched);
      } else {
        debugPrint('Fetch failed code=${res.statusCode}');
    ...
    try {
      ...
      for (final b in books) {
        try {
          ...
          final localFile = File('${imagesDir.path}/ramdarshan_$id${_extensionFromUrl(remotePath)}');
  ...
  Future<File?> _localImageFileForBook(dynamic book) async {
    try {
      ...
      final dir = await getApplicationDocumentsDirectory();
      final imagesDir = Directory('${dir.path}/$_imagesDirName');
      final candidate = File('${imagesDir.path}/ramdarshan_$id${_extensionFromUrl(remotePath)}');
